...
class DeepSetLayer(nn.Module):
    ...
    def forward(self, x : torch.Tensor) -> torch.Tensor :
        # x.shape = (B, N, C)
        # print(torch.isnan(x).any()) # -- check for nan values -- #
        # self.x= QuantileTransformer(output_distribution='normal').fit_transform(x)
        ...
        # normalization
        # Transpose to [batch_size, num_features, seq_length] -> [256, 12, 2379]
        # x = x.transpose(1, 2)
        # if self.normalization == 'batchnorm':
        #     x = self.bn(x)

        # Transpose back to [batch_size, seq_length, num_features] -> [256, 2379, 12]
        # x = x.transpose(1, 2)
...
#             layers.append(nn.LeakyReLU()) #leaky relu -originally relu was used
...
#         out = F.log_softmax(x, dim=-1) #original
